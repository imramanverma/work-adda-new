      ...
      <div className="max-w-5xl mx-auto space-y-6">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">
            Employer Escrow Payments & Ledger
          </h1>
          <p className="text-xs sm:text-sm text-slate-500 mt-1">
            Track all escrow deposits, held guarantees, released worker payouts, and Razorpay transaction IDs.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-xs">
            ...
            <p className="text-[11px] text-slate-400 mt-2">
              Released upon completed tasks
            </p>
          </div>

          <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-xs">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">
              Active Escrow Deposits
            </span>
            <div className="flex items-center justify-between mt-2">
            ...
            <p className="text-[11px] text-slate-400 mt-2">
              Safely held for ongoing contracts
            </p>
          </div>
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-xs">
            ...
            <p className="text-[11px] text-slate-400 mt-2">
              Special 0% fee promotional tier for all hirers
            </p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl border border-slate-200 shadow-xs overflow-hidden">
          <div className="p-6 border-b border-slate-100 flex items-center justify-between">
            <div>
              <h3 className="font-bold text-base text-slate-900">Payment & Escrow Ledger</h3>
              <p className="text-xs text-slate-500">Verified Razorpay gateway orders and escrow balances</p>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="p-12 text-center">
              ...
              <p className="text-xs text-slate-500">No escrow payments deposited yet.</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs">
                <thead className="bg-slate-50 text-slate-500 border-b border-slate-100 uppercase tracking-wider font-semibold text-[10px]">
                  <tr>
                    <th className="px-6 py-3.5">Date & Razorpay ID</th>
                ...
                <tbody className="divide-y divide-slate-100 text-slate-700 font-medium">
                  {payments.map((p) => {
                    ...
                    const isRefunded = p.escrowStatus === "REFUNDED";
                      ...
                      <tr key={p.id} className="hover:bg-slate-50/70 transition">
                        <td className="px-6 py-4 whitespace-nowrap text-slate-500">
                          ...
                          <span className="block text-[10px] text-slate-400 font-mono mt-0.5">
                            {p.razorpayPaymentId || p.razorpayOrderId || p.transactionId}
                          </span>
                        </td>
                        <td className="px-6 py-4 font-bold text-slate-900 max-w-xs truncate">
                          {p.job?.title || "Contract"}
                        </td>
